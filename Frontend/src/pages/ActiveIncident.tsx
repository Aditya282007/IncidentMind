...
const AGENT_ORDER: AgentName[] = ['Orchestrator', 'Watcher', 'Diagnoser', 'Patcher', 'Communicator'];
...
function CommunicatorCard({ result }: { result: CommunicatorResult }) {
  return (
    <div className="space-y-3">
      ...
      {expanded && state.result && (
        <div className="px-4 pb-4 border-t border-[#1e2d4d]">
          <div className="pt-3">
            ...
            {agent === 'Diagnoser' && <DiagnoserCard result={state.result as DiagnoserResult} />}
            {agent === 'Patcher' && <PatcherCard result={state.result as PatcherResult} />}
            {agent === 'Communicator' && <CommunicatorCard result={state.result as CommunicatorResult} />}
            {agent === 'Orchestrator' && (
...
export default function ActiveIncident({ onNavigate, initialScenarioId }: ActiveIncidentProps) {
  ...
  const runSimulation = useCallback((sc: IncidentScenario) => {
    ...
    const orchestratorComplete: SSEEvent = {
      ...
      agent: 'Orchestrator',
      result: {
        sequence: ['Watcher', 'Diagnoser', 'Patcher', 'Communicator'],
        context: {},
    ...
    <div className="flex h-screen bg-[#05081a] pt-12 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          {uiPhase === 'running' && activeAgent && (
            <div className="w-60 border-l border-[#1e2d4d] bg-[#080d1f] shrink-0 overflow-y-auto">
