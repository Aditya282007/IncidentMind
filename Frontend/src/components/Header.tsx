...
export default function Header({ currentPage, onNavigate, showSearch, searchPlaceholder }: HeaderProps) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 h-12 flex items-center px-4 border-b border-[#1e2d4d] bg-[#05081a]/95 backdrop-blur-sm">
      {/* Logo */}
      <div
        className="flex items-center gap-2 mr-8 cursor-pointer"
        onClick={() => onNavigate('dashboard')}
      >
        <div className="w-6 h-6 rounded flex items-center justify-center bg-cyan-500/20 border border-cyan-500/40">
          <Zap size={14} className="text-cyan-400" />
        </div>
        <span className="font-semibold text-white text-sm tracking-tight">IncidentMind</span>
      </div>
        ...
        {navItems.map((item) => (
          <button
            ...
            className={`px-3 py-1.5 text-xs font-medium rounded transition-all duration-150 ${
              currentPage === item.id
                ? 'text-cyan-400 bg-cyan-400/10 border-b border-cyan-400'
      ...
      {showSearch && (
        <div className="relative mx-4">
          ...
          <input
            ...
            className="pl-7 pr-3 py-1 rounded bg-[#0c1228] border border-[#1e2d4d] text-xs text-slate-300 placeholder-slate-600 focus:outline-none focus:border-cyan-500/40 w-44"
          />
        </div>
      )}
      ...
      <div className="flex items-center gap-3">
        {/* Live indicator */}
        <div className="flex items-center gap-1.5 px-2 py-1 rounded bg-[#0c1228] border border-[#1e2d4d]">
          <Radio size={10} className="text-cyan-400" />
          <span className="text-[10px] font-mono text-cyan-400 tracking-widest">LIVE</span>
        </div>
        ...
        <div ref={notifRef} className="relative">
          ...
          {showNotifs && (
            <div className="absolute right-0 top-full mt-2 w-80 rounded-xl border border-[#1e2d4d] bg-[#080d1f] shadow-2xl overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 border-b border-[#1e2d4d]">
                <div className="flex items-center gap-2">
                  ...
                  {unread > 0 && (
                    <span className="px-1.5 py-0.5 rounded-full bg-red-500/15 border border-red-500/25 text-red-400 text-[9px] font-semibold">{unread} new</span>
                  )}
                </div>
                {unread > 0 && (
                  <button onClick={markAllRead} className="text-[10px] text-cyan-400 hover:text-cyan-300 transition-colors">Mark all read</button>
                )}
              </div>
              <div className="max-h-72 overflow-y-auto divide-y divide-[#1e2d4d]">
                {notifList.length === 0 ? (
                  ...
                  <div key={n.id} className={`flex items-start gap-3 px-4 py-3 transition-colors hover:bg-white/3 ${!n.read ? 'bg-cyan-500/3' : ''}`}>
                    ...
                    <button onClick={() => dismiss(n.id)} className="text-slate-700 hover:text-slate-400 transition-colors shrink-0">
                      <X size={11} />
                    </button>
                  </div>
                ))}
        ...
        <div ref={userRef} className="relative">
          <button
            ...
            className="flex items-center gap-2 px-2 py-1 rounded border border-[#1e2d4d] bg-[#0c1228] hover:border-cyan-500/30 transition-colors"
          >
            ...
            <span className="text-xs text-slate-300 font-mono">OPERATOR_04</span>
            <ChevronDown size={10} className={`text-slate-500 transition-transform ${showUser ? 'rotate-180' : ''}`} />
          </button>

          {showUser && (
            <div className="absolute right-0 top-full mt-2 w-56 rounded-xl border border-[#1e2d4d] bg-[#080d1f] shadow-2xl overflow-hidden">
              <div className="px-4 py-3 border-b border-[#1e2d4d]">
                <div className="flex items-center gap-2.5">
                  <div className="w-8 h-8 rounded-full bg-gradient-to-br from-cyan-500/30 to-blue-600/30 border border-cyan-500/40 flex items-center justify-center">
                    <User size={14} className="text-cyan-400" />
                  </div>
                  <div>
                    <div className="text-[11px] font-semibold text-white">Julian Vane</div>
                    <div className="text-[9px] text-slate-500 font-mono">j.vane@incidentmind.io</div>
                  </div>
                </div>
              ...
              <div className="p-1.5 space-y-0.5">
                <button
                  ...
                  className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-left text-[11px] text-slate-400 hover:text-white hover:bg-white/5 transition-colors"
                >
              ...
              <div className="p-1.5 border-t border-[#1e2d4d]">
                <button
                  className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-left text-[11px] text-red-400 hover:text-red-300 hover:bg-red-500/5 transition-colors"
                >
                  <LogOut size={12} />
                  Sign Out
                </button>
